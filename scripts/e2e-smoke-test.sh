...
#
# Auth (Datum Cloud side): reuses whatever Datum Cloud session `datumctl`
# already has active via DATUM_CREDENTIALS_HELPER — no separate login needed.
# Defaults to a native Linux datumctl (a real Datum Cloud service account,
# logged in via `datumctl login --credentials <sa-creds.json> --hostname
# auth.datum.net` — see NOTES.md). Override the vars below if your setup
# differs.
#
# Auth (local daemon side): this script runs non-interactively, and the CLI
# deliberately refuses to fall back to the ambient setup token in that case
# (see resolveToken() in connect-plugin/tunnel/api/main.go) — so it reads
# the daemon's own setup token off disk and passes it explicitly via
# --token, the same way a CI job or any other scripted caller has to.
set -uo pipefail
...
# The CLI refuses to fall back to the ambient setup token when stdout isn't a
# TTY (see resolveToken() in connect-plugin/tunnel/api/main.go) — a genuine,
# deliberate guard against a non-interactive caller silently getting
# full-access credentials. This script runs non-interactively (CI, or piped
# through another tool), so it must read and pass the setup token explicitly,
# exactly the way any other non-interactive caller (a CI action, an agent)
# will need to.
SETUP_TOKEN=$(cat "$DATUM_CONNECT_DIR/daemon_auth/setup.token" 2>/dev/null)
[ -z "$SETUP_TOKEN" ] && { echo "FAILED: no setup token at $DATUM_CONNECT_DIR/daemon_auth/setup.token"; cat /tmp/daemon.log; exit 1; }
cli() { "$CLI" --port "$PORT" --token "$SETUP_TOKEN" "$@"; }

cleanup() {
  [ -n "${TUNNEL_ID:-}" ] && cli tunnel api stop "$TUNNEL_ID" >/dev/null 2>&1
  [ -n "${TUNNEL_ID:-}" ] && cli tunnel api delete "$TUNNEL_ID" >/dev/null 2>&1
...
cli tunnel api start "$TUNNEL_ID" >/dev/null

echo "=== waiting for setup pipeline (up to ~3min) ==="
HOSTNAME=""
for i in $(seq 1 60); do
  PROG=$(cli tunnel api progress "$TUNNEL_ID" 2>&1 | tr -d '\n')
  HOSTNAME=$(echo "$PROG" | grep -o '"hostnames": *\[[^]]*\]' | grep -o '"[a-zA-Z0-9.-]*\.[a-zA-Z]*"' | head -1 | tr -d '"')
  READY=$(echo "$PROG" | grep -o '"ready": *[a-z]*' | grep -o '[a-z]*$')
  TERMINAL_FAILURE=$(echo "$PROG" | grep -o '"terminal_failure": *[a-z]*' | grep -o '[a-z]*$')
  echo "[$i] ready=$READY terminal_failure=$TERMINAL_FAILURE hostname=$HOSTNAME"
  if [ "$TERMINAL_FAILURE" = "true" ]; then
    echo "FAILED: tunnel hit a terminal failure (won't self-heal by waiting longer):"
    echo "$PROG"
    exit 1
  fi
  [ "$READY" = "true" ] && [ -n "$HOSTNAME" ] && { echo "READY"; break; }
  sleep 3
done

[ -z "$HOSTNAME" ] && { echo "FAILED: never got a hostname"; cli tunnel api progress "$TUNNEL_ID"; exit 1; }
