...
func TestInstall_EndpointAliasStillWorks(t *testing.T) {
	// --endpoint is a deprecated alias for --origin, kept for tickets/docs
	// that reference it. It must still satisfy the origin requirement.
	pluginBin := buildPlugin(t)
	...
	if strings.Contains(string(out), "--origin is required") {
		t.Errorf("--endpoint alias should satisfy the origin requirement:\n%s", out)
	}
