...
//! |----------------------|-------------------------------|-----------------------------------------------------------------|
//! | `tunnel_progress`    | per step status transition    | `step` (snake_case kind), `status`, `resource` (Option<String>), `reason` (Option<String>, the underlying condition's `reason`), `message` (Option<String>, the underlying condition's `message` — e.g. a masked quota/scheduling failure the condition's `status`/`reason` alone wouldn't surface), `step_elapsed_secs`/`total_elapsed_secs` (the stderr line's `(step/total)` notation, also given to JSON consumers) |
...
pub fn render_progress_step(
    ...
    if mode == Mode::Json {
        let v = serde_json::json!({
            ...
            // Both halves of the stderr line's "(step/total)" notation
            // above — previously only ever written there, and only on the
            // Ready transition. Forwarded here for every status so a
            // caller watching JSON output can show how long a step has
            // been sitting Pending, not just how long a completed one took.
...
mod tests {
    ...
    fn json_progress_event_parses_back_to_expected_fields() {
        ...
        let step_elapsed = Duration::from_secs_f64(1.5);
        let total_elapsed = Duration::from_secs_f64(12.0);
        let v = serde_json::json!({
            "type": "tunnel_progress",
